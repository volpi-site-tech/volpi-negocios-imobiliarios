...
fetch('data.json').then(r=>r.json()).then(function(data){
  ...
  featured.forEach(function(slug){ var p = props.find(x=>x.slug===slug); if(p) featuredGrid.appendChild(makeCard(p)); });
  props.forEach(function(p){ allGrid.appendChild(makeCard(p)); });
});
